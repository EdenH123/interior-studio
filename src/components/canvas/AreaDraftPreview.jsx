...
// stage's area-point handler.
export default function AreaDraftPreview({ draft, cursor, scale }) {
  ...
  return (
    <>
      {closeable && (
        <Line points={pts} closed fill="rgba(132,204,22,0.14)" listening={false} />
      )}
      <Line points={pts} stroke="#84cc16" strokeWidth={2 / scale}
        dash={[6 / scale, 4 / scale]} listening={false} />
      {cursor && (
        <Line points={[last.x, last.y, cursor.x, cursor.y]} stroke="#84cc16"
