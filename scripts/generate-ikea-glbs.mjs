...
writeGLB('ikea-kallax-2x2', [
  // Outer shell panels
  box(-0.385, 0,    -0.195, 0.385, 0.02,  0.195),  // bottom
  box(-0.385, 0.75, -0.195, 0.385, 0.77,  0.195),  // top
  box(-0.385, 0.02, -0.195,-0.365, 0.75,  0.195),  // left side
  box( 0.365, 0.02, -0.195, 0.385, 0.75,  0.195),  // right side
  box(-0.385, 0,    -0.195, 0.385, 0.77, -0.175),  // back panel
  // Inner dividers
  box(-0.01,  0.02, -0.175, 0.01,  0.75,  0.195),  // vertical mid-divider
  box(-0.365, 0.375,-0.175, 0.365, 0.395, 0.195),  // horizontal mid-divider
], C_IKEA_WHITE)

// KALLAX 4×2  W1.47 × D0.39 × H0.77
writeGLB('ikea-kallax-4x2', [
  box(-0.735, 0,    -0.195, 0.735, 0.02,  0.195),
  box(-0.735, 0.75, -0.195, 0.735, 0.77,  0.195),
  box(-0.735, 0.02, -0.195,-0.715, 0.75,  0.195),
  box( 0.715, 0.02, -0.195, 0.735, 0.75,  0.195),
  box(-0.735, 0,    -0.195, 0.735, 0.77, -0.175),
  // 3 vertical dividers evenly spaced across 1.47 m (4 columns)
  box(-0.378, 0.02, -0.175,-0.358, 0.75,  0.195),
  box(-0.010, 0.02, -0.175, 0.010, 0.75,  0.195),
  box( 0.358, 0.02, -0.175, 0.378, 0.75,  0.195),
  // horizontal mid-divider
  box(-0.715, 0.375,-0.175, 0.715, 0.395, 0.195),
], C_IKEA_WHITE)

// BILLY bookcase  W0.80 × D0.28 × H2.02  (open front = +Z)
writeGLB('ikea-billy', (() => {
  const parts = [
    box(-0.40, 0,    -0.14,  0.40, 0.02,  0.14),   // bottom
    box(-0.40, 2.00, -0.14,  0.40, 2.02,  0.14),   // top
    box(-0.40, 0.02, -0.14, -0.38, 2.00,  0.14),   // left side
    box( 0.38, 0.02, -0.14,  0.40, 2.00,  0.14),   // right side
    box(-0.40, 0,    -0.14,  0.40, 2.02, -0.12),   // back panel
  ]
  // 4 adjustable shelves at equal spacing
  const shelfY = [0.38, 0.76, 1.14, 1.52]
  for (const y of shelfY) {
    parts.push(box(-0.38, y, -0.12, 0.38, y + 0.018, 0.14))
  }
  return parts
})(), C_BIRCH)
...
writeGLB('ikea-pax-100', [
  box(-0.50, 0,    -0.29,  0.50, 0.02,  0.29),    // bottom
  box(-0.50, 1.99, -0.29,  0.50, 2.01,  0.29),    // top
  box(-0.50, 0.02, -0.29, -0.48, 1.99,  0.29),    // left side
  box( 0.48, 0.02, -0.29,  0.50, 1.99,  0.29),    // right side
  box(-0.50, 0,    -0.29,  0.50, 2.01, -0.27),    // back panel
  // hanging rod
  cyl(0, 0, 0.55, 1.95, 0.012, 8),
  // 2 doors (full-height)
  box(-0.48, 0.02,  0.28, -0.04, 1.97,  0.30),    // left door
  box( 0.04, 0.02,  0.28,  0.48, 1.97,  0.30),    // right door
