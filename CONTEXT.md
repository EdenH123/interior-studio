...
- [x] Fix: falling through rotated stairs in 3D walkthrough (2026-05-30)
  - `stairGroundY` in `walkthroughCollision.js` was computing the world→local rotation with the wrong matrix sign: `localX = cosY·dx + sinY·dz; localZ = −sinY·dx + cosY·dz` (which is `R_Y(+yaw)`, the forward transform, not the inverse). For any non-axis-aligned stair, the player's footprint check ran in the wrong frame and `frac = (localZ + depth/2)/depth` returned the height at the OPPOSITE end of the ramp — top reported as bottom and vice versa. groundY stayed at floor level while the rendered geometry sat 2.4 m up, so gravity yanked the camera straight through the mesh. The existing rotated-stair unit test masked the bug because its assertions were written against the same wrong convention. Fix: correct formulas are `localX = cos(yaw)·dx − sin(yaw)·dz`, `localZ = sin(yaw)·dx + cos(yaw)·dz` (= `R_Y(−yaw)`). Test updated + a new case added for `yaw = −π/2` (the value produced by Konva +90° via `konvaRotationToThreeY`) so the actual user flow is regression-locked.
  - Build green, 530 / 530 tests passing.

- [x] Vertical placement: manual elevation + stack-on-furniture (TV on wardrobe) (2026-05-30)
  - Two new optional fields on furniture items: `elevation` (metres above the level floor, manual override) and `stackedOn` (furniture id — child sits on top of parent). Both feed into `resolveStackedYOffset(f, byId, getAutoOffset)` in `src/components/canvas/stackGeometry.js`, whose resolution order is: stack chain (recursive `parent.y + parent.height`) → `elevation` → `mountHeight` (wall-mounted) → lighting auto-offset → 0. Cycle-safe via a visited set; missing parent → falls back to floor.
  - 3D reconciler (`reconcileFurniture.js`) now calls `resolveStackedYOffset` instead of the old inline `f.wallMounted ? f.mountHeight : lightYOffset(...)` branch — same behaviour for existing items (no schema change required for old projects), new fields just join the resolution chain.
  - 2D stacking on drag: `Furniture.jsx` calls `findStackTarget(world, item, furniture)` inside `onDragMove`. Stack-on-furniture wins over wall snap (no pulling to a wall edge when the cursor is inside another item's footprint). On `onDragEnd`, sets `stackedOn: parentId`, inherits parent's `levelId`, and clears any manual `elevation` so the stack chain owns Y unambiguously. `findStackTarget` filters out lights, wall-mounted items, stairs, railings, items on a different level, descendants of the dragging item (cycle protection), and the dragging item itself; prefers the smallest-footprint candidate for nested stacks.
  - Sidebar drop (`useFurnitureDrop.js`) honours the same precedence — drop directly on top of another furniture creates the new item with `stackedOn` set; the drag ghost (`DragGhost.jsx`) shows an orange outline + "↑ stack" badge when stacking is active.
  - Multi-drag is exempt: `Furniture.jsx onDragStart` reads the current selection and disables stack-detect when more than one furniture item is selected, so group-translate (the PR #86 flow) never accidentally re-parents a group member.
  - PropertiesPanel (`FurnitureProps.jsx`): non-wall-mounted, non-stair, non-railing items now show either an `ElevationField` (numeric m above floor, 0 stored as null so the item falls back to its auto-offset) or a `StackedOnRow` with the parent's label + "Unstack" button. Translations added in `en` + `he`.
  - Cascade: `removeFurniture` clears `stackedOn` on any orphaned children in the same `set()`. They keep their last-rendered x/y, just drop to the floor in 3D.
  - Tests: new `stackGeometry.test.js` (17 cases: footprint containment with rotation, level filtering, descendant-cycle protection, smallest-candidate preference; resolution-order matrix incl. cycle safety). Total: 529 / 529 passing.
